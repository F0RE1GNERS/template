...
#define rson o * 2 + 1, m + 1, r
    ...
    LL query(int p, int q, int o, int l, int r, LL addv = 0) {
        if (p > r || l > q) return 0;
        if (p <= l && r <= q) return sum[o] + addv * (r - l + 1);
        int m = (l + r) / 2;
        return query(p, q, lson, addv + add[o]) +
               query(p, q, rson, addv + add[o]);
    }
} IT;
```
...
+ find_by_order 找 order + 1 小的元素 （其实都是从 0 开始计数）
